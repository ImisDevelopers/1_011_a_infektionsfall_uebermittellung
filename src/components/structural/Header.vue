<template>
  ...
  >
    ...
    >
      <a slot="content">
        <a-menu mode="inline" :selectedKeys="[]" style="border: none;">
          <a-menu-item key="1" class="menu-item">
            <router-link :to="{ name: 'account' }" @click.native="routeClicked">
              <a-icon type="user" />
              <span class="nav-text">Benutzerkonto</span>
            </router-link>
          </a-menu-item>
          <a-menu-item @click="onLogout" class="menu-item">
            <a-icon type="logout" />
            <span class="nav-text">
              Abmelden
            </span>
      ...
      <div style="flex: 0 0 auto; cursor: pointer;">
        <a-icon style="color: #aaa; padding: 1.5rem;" type="user" />
        <span
          class="username"
          style="margin-right: 15px; color: rgba(255, 255, 255, 0.87);"
          >{{ username }}</span
        >
        <a-icon
